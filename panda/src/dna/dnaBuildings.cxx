...
float DNAWall::current_wall_height = 0.0;
...
NodePath DNAWall::traverse(NodePath &parent, DNAStorage *store, int editing) {
  ...
  _pos.set_z(DNAWall::current_wall_height);
  ...
  DNAWall::current_wall_height += _height;
...
NodePath DNAFlatBuilding::traverse(NodePath &parent, DNAStorage *store, int editing) {
  ...
  if (DNAWall::current_wall_height == 0.0) {
  ...
  wall_camera_barrier_node_path.set_scale(1.0, 1.0, DNAWall::current_wall_height);

  // Build origin for suit flat building:
  setup_suit_flat_building(parent, store);

  // Build origin for cogdo flat building:
